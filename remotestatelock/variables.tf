...
variable "dynamoDB_name" {
  description = "DynamoDB Name for state lock storage"
  type        = string
  default     = "tfstateStore"
  validation {
    condition     = length(var.dynamoDB_name) >= 4
    error_message = "The DynamoDB Name must be 4 characters or more in length."
  }
}